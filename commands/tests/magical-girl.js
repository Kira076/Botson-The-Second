...
const validTransforms = transformationFiles.splice(transformationFiles.indexOf('test.json')).forEach(file => {validArgs.push(file.slice[0, -5])});
...
module.exports = class MeowCommand extends Command {
    ...
    run(message, { transformation }) {
        const transform = require(`../../transformations/${transformation}.json`);
        const channelList = [];
        (async () => {
            try {
                for (const channel in transform) {
                    await message.channel.guild.channels.cache.get(channel)
                        .setName(transform[channel]);
                    channelList.push(transform[channel]);
                }
            }
            catch (err) {
        ...
        return message.say('Moon... Prism... POWER!');
    }
};